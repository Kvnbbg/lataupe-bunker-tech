...
But technology is only as reliable as the people maintaining it. When critical systems fail, it's up to you and your team to diagnose problems, implement solutions, and keep the community functioning.'''
            },
            {
                'id': 'chapter3',
...
Emergency drills are a weekly occurrence. Every resident knows their role in case of system failures, contamination alerts, or the dreaded possibility of surface breach.'''
            },
            {
                'id': 'epilogue',
...
Les signes étaient indéniables : les niveaux de radiation UV montèrent en flèche, les températures de surface devinrent insupportables pendant les heures de jour, et l'air extérieur lui-même devint hostile à la vie humaine.
...
Les ordinateurs du bunker fonctionnent sur des systèmes durcis, résistants aux interférences électromagnétiques. La communication avec d'autres complexes de bunkers repose sur une combinaison de réseaux de fibre souterrains et de systèmes radio fortement blindés.

Mais la technologie n'est fiable que tant que les gens qui la maintiennent le sont.'''
            },
            {
                'id': 'chapter3',
                'title': 'Chapitre 3: La Vie Souterraine',
                'content': '''Six mois après le début des opérations du bunker, des routines ont émergé. Les résidents travaillent par équipes pour maintenir les machines complexes qui les gardent en vie. Les baies hydroponiques fournissent de la nourriture fraîche, les systèmes de recyclage d'eau assurent que rien n'est gaspillé.
...
Mais la psychologie humaine n'était pas conçue pour une vie souterraine permanente. Dépression, anxiété, et claustrophobie sont des menaces constantes. Les zones récréatives du bunker, les systèmes de ciel artificiel, et les espaces communautaires sont aussi vitaux que les systèmes de support de vie eux-mêmes.'''
            },
            {
                ...
                'content': '''Alors que vous révisez les données environnementales de la journée, vous réfléchissez à l'incroyable adaptabilité de l'humanité. Le bunker qui semblait être un refuge temporaire devient un foyer permanent.

La surface peut être perdue, mais l'espoir vit dans les profondeurs.'''
            }
        ]
    }
}
...
def create_sample_data():
    ...
    if EnvironmentalsData.query.first():
        return
    ...
    for i in range(100):
        ...
        data = EnvironmentalsData(
            timestamp=timestamp,
            temperature=18 + random.uniform(-3, 6),
            humidity=45 + random.uniform(-10, 20),
            air_quality=85 + random.uniform(-15, 10),
            oxygen_level=20.5 + random.uniform(-1, 0.5),
            co2_level=400 + random.uniform(-50, 200),
            radiation_level=0.1 + random.uniform(0, 0.5),
            atmospheric_pressure=1013 + random.uniform(-20, 20),
            bunker_id='bunker-alpha-7',
            sensor_location=f'Sector-{random.choice(["A", "B", "C"])}{random.randint(1, 5)}'
        )
        db.session.add(data)
    ...
    alerts = [
        Alert(
            ...
            severity='medium',
            message='Temperature rising in Sector B3 - check ventilation system',
            bunker_id='bunker-alpha-7',
            sensor_location='Sector-B3'
        ),
        Alert(
            alert_type='radiation',
            severity='low',
            message='Minor radiation spike detected - within normal parameters',
            bunker_id='bunker-alpha-7',
            sensor_location='Perimeter-N1'
        )
    ]
